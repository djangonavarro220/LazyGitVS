...
F           Open Create fixup commit → f/a/r (bounded fixup/amend/reword)
...
Ctrl+J/K    Move the selected ordinary commit/range down/up one position
...
LazyGitVS also has **partial Commits Create fixup commit parity**: configured `keybinding.commits.createFixupCommit` (default `F`) is routed only from the attached top-level Local Commits list and opens the exact native **Create fixup commit** menu in `f`/`a`/`r` order: **fixup! commit**, **amend! commit with changes**, and **amend! commit without changes (pure reword)**. The selected ordinary reachable non-merge commit, attached branch, full message, HEAD, repository, staged/unstaged/untracked snapshot, active-operation/conflict state, and `commit.gpgSign` setting are captured and revalidated before every mutation. `f` uses exact `git commit --fixup=<fullhash>` over staged files; `a` and `r` load the full selected message through native **Create "amend!" commit** and **Commit description** inputs and use exact `amend! <subject>` message construction, with `r` adding `--only --allow-empty`. When changes exist but none are staged, `f`/`a` ask **No files staged** / **You have not staged any files. Commit all files?** before staging all; with no files those entries are disabled with **No files staged**, while `r` remains enabled. Success verifies one new HEAD child, exact message/tree, and returns the target selection index plus one; cancellation and drift are read-only, while failed Git state is surfaced without claiming rollback.
...
LazyGitVS also has **partial Commits move down/up parity**: configured `keybinding.commits.moveDownCommit` / `keybinding.commits.moveUpCommit` (defaults `<ctrl+j>` / `<ctrl+k>`) move one selected ordinary commit or contiguous visible range exactly one displayed position down/up from attached top-level Local Commits. After an initial and immediate second full preflight, with no modal or terminal, it reports **Moving** and runs exactly one `git rebase --interactive --autostash --keep-empty --no-autosquash --rebase-merges <base|--root>` with `GIT_EDITOR=true` and a private 0700 C-locale sequence editor. The editor swaps only the selected generated `pick` block across its one adjacent ordinary `pick`, leaves comments and unrelated todo directives byte-for-byte, and fails closed on ambiguous hashes. Dirty staged/unstaged/untracked state, active operations, detached or branch-scoped views, filtered/nonlinear/duplicate/unreachable selections, merge selections or destinations, `commit.gpgSign=true`, and visible top/bottom bounds are rejected before **Moving**. Success verifies the rewritten first-parent ordering and shifts the numerical selection/range one slot without claiming old hashes survive; conflicts remain active for Status recovery and no-operation failures surface Git's error.

This is deliberately bounded rather than full lazygit parity: a dirty target worktree (including staged, unstaged, or untracked changes), an active merge/rebase/cherry-pick/revert, detached HEAD, a different branch-scoped Commits view, selected merge commits, unreachable/noncontiguous/duplicate selected hashes, and a selected root/range with no commit below for Drop/Squash/Fixup are rejected before mutation. Edit accepts root and intentional-empty commits but excludes active-rebase todo editing, merge commits, dirty-tree auto-stash, generic `i`, and editor-based broader rebase UI; if a failure leaves any operation active, LGVS preserves it and directs recovery through Status. Move remains limited to one adjacent ordinary first-parent swap in the unfiltered visible prefix; active-todo editing, merge movement, dirty auto-stash, and a general rebase UI remain open. Full merge Drop, Drop while already rebasing, and dirty-worktree auto-stash remain open. For Squash-down and Fixup, active-rebase todo edits, selected merge handling, dirty-worktree auto-stash, and broader rebase UI (editor-based reword/edit/move) remain open. Apply fixup commits deliberately omits lazygit's branch/main-status-dependent `b` action and broader autosquash/rebase UI. Reword deliberately excludes visual ranges, merge commits, `commit.gpgSign=true`, dirty-tree auto-stash, and the editor-based `R` variant. Cancellation, preflight rejection, and input-time repository drift are read-only; a Git failure without an active rebase is surfaced rather than described as a rollback. The existing cherry-pick limits also remain: merge-mainline (`-m 1`) handling, Git-version-specific empty-commit flags, and broader cross-panel range behavior are deferred. After a successful Drop, Squash-down, Fixup, Reword, or Move, the selection returns to the computed numerical location when it still exists (otherwise clamped after refresh).
