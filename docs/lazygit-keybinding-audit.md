...
- `<ctrl+j>` (configured `keybinding.commits.moveDownCommit`, default `<ctrl+j>`): **partial Move commit down one parity** for one selected ordinary commit or contiguous visible range in attached top-level Local Commits. It has no modal or terminal: two identical clean/attached/ordinary/linear-first-parent preflights precede **Moving** and one native interactive rebase, whose private 0700 C-locale editor swaps only the selected `pick` block across the adjacent ordinary older pick.
- `<ctrl+k>` (configured `keybinding.commits.moveUpCommit`, default `<ctrl+k>`): **partial Move commit up one parity** with the symmetric newer adjacent ordinary pick. Both directions reject dirty trees, active operations, detached/branch-scoped/filtered/nonlinear views, duplicate or unreachable selections, selected/destination merges, `commit.gpgSign=true`, and visible bounds before mutation; comments and unrelated todo directives stay byte-for-byte, success restores the shifted numeric range shape, and conflicts remain in Status recovery.
...
- `F` (configured `keybinding.commits.createFixupCommit`, default `F`): **partial Create fixup commit parity** only in attached top-level Local Commits. The native **Create fixup commit** menu preserves upstream order and exact labels: `f` **fixup! commit**, `a` **amend! commit with changes**, `r` **amend! commit without changes (pure reword)**. `f` uses `git commit --fixup=<fullhash>` over staged files; `a`/`r` capture the selected full message, open native **Create "amend!" commit** and **Commit description** inputs, and use `git commit -m "amend! <original subject>" -m <summary + optional blankline/body>`, with `r` adding `--only --allow-empty`. `f`/`a` show disabled reason **No files staged** when there are no files; when changes exist but none are staged they use the exact **No files staged** / **You have not staged any files. Commit all files?** stage-all confirmation, while `r` remains enabled. Repository/branch/HEAD/full hash/message/staged/unstaged/untracked/conflict/operation/GPG state is revalidated after the menu and each input/confirmation before **Creating fixup commit**; success verifies one child and returns selection +1. Commit-files, Files, and HUNK/LINE keep their own routes.
...
- Commits has **partial Edit parity** through configured `universal.edit`: it starts a real interactive rebase immediately for the current ordinary commit or contiguous visible range and deliberately leaves the selected `edit` stop(s) active for normal Files/amend plus Status continue/abort recovery. Active-todo editing, merges, dirty-tree auto-stash, generic `i`, and full rebase UI remain gaps; this must not be read as full upstream Edit parity.
- Commits has **partial Move down/up parity** through configured `keybinding.commits.moveDownCommit` / `keybinding.commits.moveUpCommit`: it is one adjacent ordinary first-parent swap only, not active-todo editing, merge movement, dirty-tree auto-stash, or a general rebase UI.
