...
- This `d` is scoped only to Commit-files; top-level Commits retains configured `universal.remove` Drop, while Files and HUNK/LINE retain their existing discard behavior. Visual-range commit-files, active-rebase todo editing, dirty-tree auto-stash, generic rebase UI, copy menus, patch mode, and external diff remain gaps.
