...
Audited from the available upstream lazygit checkout at exact commit `146f00491820055f3a6c0d492447d7e2b9da7d83` (`Revert the change of <ctrl+j>/<ctrl+k> to <alt+down>/<alt+up> (#5582)`). The checkout was clean apart from an unrelated untracked `.DS_Store`; every citation below was revalidated against the committed file contents at that object. Upstream source is the product specification for this implementation.

## Binding, labels, and scope

- `pkg/config/user_config.go:485-492,965-976` defines universal `undo` / `redo` and defaults them to `z` / `Z`.
...
- `pkg/i18n/english.go:1326-1331` supplies the exact `Undo` / `Redo` labels and reflog tooltips.
- `docs/keybindings/Keybindings_en.md:33-37` describes both as reflog operations over commits rather than working-tree changes.
- `docs/keybindings/Keybindings_en.md:208-214` separately assigns `z` in merge-conflict resolution to undo the last conflict resolution. LGVS therefore does not route reflog undo from its Conflicts surface.
...
- `pkg/commands/git_commands/reflog_commit_loader.go:25-36` loads the complete HEAD reflog with `git log -g`, not an arbitrary UI-sized limit.
...
- `pkg/i18n/english.go:1758-1759` provides `Can't undo while rebasing` / `Can't redo while rebasing`.
- `pkg/i18n/english.go:1942-1944` provides the exact hard-reset-with-autostash, soft-reset, and checkout-with-autostash confirmation text.
