...
async function lazyGitPreviewTabLabels(Runtime) {
  const r = await Runtime.evaluate({ expression: `Array.from(document.querySelectorAll('.tab:not(.tab .tab)')).map(el => el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent || '').map(s => s.replace(/\s+/g, ' ').trim()).filter(s => /^LazyGitVS\b/.test(s) || s.includes(' LazyGitVS'))`, returnByValue: true });
