...
async function lazyGitPreviewTabLabels(Runtime) {
  const r = await Runtime.evaluate({ expression: `Array.from(document.querySelectorAll('.tabs-container .tab[role="tab"], .tabs-and-actions-container .tab[role="tab"]')).map(el => el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent || '').map(s => s.replace(/\s+/g, ' ').trim()).filter(s => /^LazyGitVS\b/.test(s) || s.includes(' LazyGitVS'))`, returnByValue: true });
  return Array.from(new Set(r.result.value || []));
}
async function editorTabLabels(Runtime) {
  const r = await Runtime.evaluate({ expression: `Array.from(document.querySelectorAll('.tabs-container .tab[role="tab"], .tabs-and-actions-container .tab[role="tab"]')).map(el => el.getAttribute('aria-label') || el.getAttribute('title') || el.textContent || '').map(s => s.replace(/\s+/g, ' ').trim()).filter(Boolean)`, returnByValue: true });
  return Array.from(new Set(r.result.value || []));
}
