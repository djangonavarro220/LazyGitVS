...
import { EMPTY_COMMIT_RANGE, clampCommitRange, extendNonStickyCommitRange, findCommitIndexByHash, hasVisibleCopiedCommit, isCommitInRange, moveCommitSelection, pasteCopiedCommits as pasteCherryPickBuffer, resetCherryPickBuffer, toggleCopiedCommitRange, toggleStickyCommitRange, type CherryPickBuffer, type CommitRangeSelection } from './commitCherryPick';
...
class LazyGitVSController {
  ...
  private async runCommitCommand(typed: string) {
    ...
    const k = this.lazygitKeymap.commits;
    const key = (value: string | string[] | undefined) => this.keyLabel(value);
    ...
    const cherryPickBufferAction = !!findMenuItemByKey([{ key: key(k.cherryPickCopy) || 'C', label: 'Copy/toggle commit range' }, { key: key(k.pasteCommits) || 'V', label: 'Paste copied commits' }, { key: key(k.resetCherryPick) || '<ctrl+r>', label: 'Reset copied commits' }], typed); if (cherryPickBufferAction && this.commitFilesFor) return; const item = findMenuItemByKey(this.commitCommandCatalog(), typed); if (!item) return;
    try {
      await executeGitMenuItem(item);
      if (!cherryPickBufferAction) await this.refresh(false);
    } finally {
      await this.restorePanelFocusAfterModal('commits');
    }
  }
