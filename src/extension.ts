...
class LazyGitVSController {
  ...
  attach(panel: ViewPanel, view: vscode.WebviewView) {
    ...
    view.webview.onDidReceiveMessage(async rawMessage => {
      try {
        ...
        if (type === 'checkoutCommitFile') await this.checkoutCurrentCommitFile();
  ...
  private help(panel: Panel): string {
    ...
    if (panel === 'commits') {
      ...
      if (this.commitFilesFor) return common + `${kb(String(cf.checkoutCommitFile))} checkout · ${kb(String(u.goInto))} open/toggle`;
