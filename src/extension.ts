...
class LazyGitVSController {
  ...
  private async moveBlock(viewPanel: ViewPanel, delta: number) {
    ...
    if (panel === 'hunks') {
      if (this.hunkSelectionMode === 'line') {
        ...
        this.hunkLineSelected = clamp(this.hunkLineSelected + delta, len);
      } else {
        this.hunkSelected = clamp(this.hunkSelected + delta, this.hunks.length);
      }
      this.persistNavigationState();
      this.renderAll();
      await this.openCurrent('files', true).catch(() => undefined);
      return;
    }
    await this.move(viewPanel, delta);
  }
  ...
  private render(viewPanel: ViewPanel) {
    ...
    try { view.webview.html = `<!doctype html><html><head><meta charset="UTF-8"><style>
      ...
      .root.wrap-staging .row.hunk{height:auto;min-height:20px;white-space:normal;align-items:start;padding-top:2px;padding-bottom:2px}.root.wrap-staging .row.hunk .path{overflow:visible;text-overflow:clip;white-space:normal}.row{display:grid;grid-template-columns:7px 52px minmax(0,1fr) minmax(0,24%);align-items:center;column-gap:4px;height:20px;padding:0 4px;box-sizing:border-box;white-space:nowrap;min-width:0;color:var(--vscode-list-foreground,var(--vscode-foreground));cursor:default;border-left:2px solid transparent;}.row.file{grid-template-columns:7px 42px minmax(0,1fr);}.row.branch{grid-template-columns:7px 26px minmax(0,1fr) minmax(0,72px);}.row.commit{grid-template-columns:7px 48px minmax(0,1fr) minmax(0,58px);}.row.sel{background:var(--vscode-list-activeSelectionBackground);color:var(--vscode-list-activeSelectionForeground);outline:1px solid var(--vscode-list-focusOutline);outline-offset:-1px;}.row.range:not(.sel){background:var(--vscode-list-inactiveSelectionBackground);}.row:not(.sel):hover{background:var(--vscode-list-hoverBackground);}.cursor{width:7px;color:var(--vscode-focusBorder);}.status{color:var(--vscode-descriptionForeground);font-size:11px;overflow:hidden;text-overflow:clip;}.path,.summary{overflow:hidden;text-overflow:ellipsis;min-width:0;}.meta{opacity:.68;margin-left:2px;overflow:hidden;text-overflow:ellipsis;justify-self:end;min-width:0;font-size:10px;}.empty{color:var(--vscode-descriptionForeground);padding:4px 6px;}
      .status-pair{display:grid;grid-template-columns:15px 15px;column-gap:4px;align-items:center}.slot{display:inline-grid;place-items:center;min-width:15px;height:15px;border-radius:3px;font-size:10px;font-weight:800;line-height:1;border:1px solid transparent;box-sizing:border-box}.slot.empty{visibility:hidden;background:transparent;border-color:transparent;box-shadow:none}.slot.index{color:var(--vscode-gitDecoration-addedResourceForeground,#6a9955);border-color:color-mix(in srgb,var(--vscode-gitDecoration-addedResourceForeground,#6a9955) 55%,transparent);background:color-mix(in srgb,var(--vscode-gitDecoration-addedResourceForeground,#6a9955) 16%,transparent)}.slot.worktree{color:var(--vscode-gitDecoration-modifiedResourceForeground,#e06c75);border-color:color-mix(in srgb,var(--vscode-gitDecoration-modifiedResourceForeground,#e06c75) 55%,transparent);background:color-mix(in srgb,var(--vscode-gitDecoration-modifiedResourceForeground,#e06c75) 16%,transparent)}.slot.deleted,.slot.conflict{color:var(--vscode-errorForeground,#f85149);border-color:color-mix(in srgb,var(--vscode-errorForeground,#f85149) 60%,transparent);background:color-mix(in srgb,var(--vscode-errorForeground,#f85149) 18%,transparent)}.slot.untracked{color:var(--vscode-gitDecoration-untrackedResourceForeground,#d7ba7d);border-color:color-mix(in srgb,var(--vscode-gitDecoration-untrackedResourceForeground,#d7ba7d) 60%,transparent);background:color-mix(in srgb,var(--vscode-gitDecoration-untrackedResourceForeground,#d7ba7d) 16%,transparent)}.row.file.staged{border-left-color:var(--vscode-gitDecoration-addedResourceForeground,#6a9955)}.row.file.unstaged{border-left-color:var(--vscode-gitDecoration-modifiedResourceForeground,#e06c75)}.row.file.mixed{border-left-color:var(--vscode-gitDecoration-conflictingResourceForeground,#569cd6)}.row.file.untracked{border-left-color:var(--vscode-gitDecoration-untrackedResourceForeground,#d7ba7d)}.row.file.staged .path{color:var(--vscode-gitDecoration-addedResourceForeground,var(--vscode-foreground))}.row.file.unstaged .path{color:var(--vscode-gitDecoration-modifiedResourceForeground,var(--vscode-foreground))}.row.file.untracked .path{color:var(--vscode-gitDecoration-untrackedResourceForeground,var(--vscode-foreground))}.row.sel .slot{color:inherit}.row.sel .slot.index{color:var(--vscode-gitDecoration-addedResourceForeground,#6a9955)}.row.sel .slot.worktree{color:var(--vscode-gitDecoration-modifiedResourceForeground,#e06c75)}.row.sel .slot.untracked{color:var(--vscode-gitDecoration-untrackedResourceForeground,#d7ba7d)}.row.sel .slot.deleted,.row.sel .slot.conflict{color:var(--vscode-errorForeground,#f85149)}.current .path,.current .status{font-weight:700}.danger .status{color:var(--vscode-errorForeground)}.hint{color:var(--vscode-descriptionForeground);font-size:11px;line-height:1.35;padding:4px 8px 5px;border-top:1px solid var(--vscode-sideBarSectionHeader-border);background:var(--vscode-sideBar-background);}kbd{font-family:var(--vscode-editor-font-family);font-size:10px;color:var(--vscode-keybindingLabel-foreground);background:var(--vscode-keybindingLabel-background);border:1px solid var(--vscode-keybindingLabel-border);border-bottom-color:var(--vscode-keybindingLabel-bottomBorder);border-radius:3px;padding:0 3px;margin-right:2px;}.statusline{color:var(--vscode-descriptionForeground);padding-top:3px;}
      ...
      window.addEventListener('keydown',e=>{ const u=keymap.universal, f=keymap.files, m=keymap.main; const jump=Array.isArray(u.jumpToBlock)?u.jumpToBlock:[]; const jumpIndex=jump.indexOf(e.key); const jumpPanel = jumpIndex>=0 ? panels[String(jumpIndex+1)] : panels[e.key]; if(jumpPanel){e.preventDefault();vscode.postMessage({type:'switchPanel',panel:jumpPanel});return;}
        ...
        if((panel==='files'||panel==='hunks')&&hit(e,u.nextBlock,u.nextBlockAlt)){e.preventDefault();vscode.postMessage({type:'moveBlock',delta:1});return;} if((panel==='files'||panel==='hunks')&&hit(e,u.prevBlock,u.prevBlockAlt)){e.preventDefault();vscode.postMessage({type:'moveBlock',delta:-1});return;}
  ...
  private help(panel: Panel): string {
    ...
    if (panel === 'status') return common + `${kb(String(u.goInto))} files · ${kb(String(u.push))}/${kb(String(u.pull))} push/pull · ${kb(String(f.stashAllChanges))} stash · ${kb(String(f.viewResetOptions))} nuke`;
  ...
  private renderStatus(): string {
    ...
    const changed = this.files.filter(f => f.xy[1] !== ' ' && !f.untracked).length;
    const untracked = this.files.filter(f => f.untracked).length;
    return [
      row(false, 'current', 'repo', repo, workspaceRoot()),
      row(false, 'current', 'branch', branch),
      row(false, staged ? 'staged' : '', 'staged', String(staged)),
      row(false, changed ? 'unstaged' : '', 'changed', String(changed)),
      row(false, untracked ? 'untracked' : '', 'new', String(untracked)),
      row(false, this.conflictItems.length ? 'danger' : '', 'conflicts', String(this.conflictItems.length)),
      row(false, '', 'stash', String(this.stashItems.length)),
      row(false, '', 'tags', String(this.tagItems.length)),
      row(false, '', 'remotes', String(this.remoteItems.length)),
      row(false, '', 'repos', String(this.workspaceRepos.length)),
      row(false, '', 'filter', this.fileStatusFilter),
      row(false, '', 'search', this.filterText || 'none'),
      row(false, '', 'gui', `bottom:${this.lazygitGui.showBottomLine ? 'on' : 'off'} jumps:${this.lazygitGui.showPanelJumps ? 'on' : 'off'} tree:${this.lazygitGui.showFileTree ? 'on' : 'off'} sort:${this.lazygitGui.fileTreeSortOrder}`),
      row(false, '', 'staging', `wrap:${this.lazygitGui.wrapLinesInStagingView ? 'on' : 'off'} mode:${this.lazygitGui.useHunkModeInStagingView ? 'hunk' : 'line'}`),
      row(false, '', 'git', `ctx:${this.lazygitGit.diffContextSize} whitespace:${this.lazygitGit.ignoreWhitespaceInDiffView ? 'ignored' : 'shown'} rename:${this.lazygitGit.renameSimilarityThreshold}%`),
      row(false, '', 'config', this.lazygitConfigFiles.length ? this.lazygitConfigFiles.map(f => path.basename(f)).join(', ') : 'defaults')
    ].join('');
  }
}
