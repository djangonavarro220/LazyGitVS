...
import { chooseApplyFixupCommitsAction, commitAutosquashMenuItem } from './commitAutosquash'; import { commitEditMenuItem } from './commitEdit'; import { commitRewordMenuItem } from './commitReword';
...
class LazyGitVSController {
  ...
  private help(panel: Panel): string {
    ...
    if (panel === 'commits') {
      ...
      return common + `${kb(String(u.edit))} edit · ${kb(String(u.remove))} drop · ${kb(String(c.squashDown))} squash · ${kb(String(c.squashAboveCommits))} apply fixups · ${kb(String(c.markCommitAsFixup))} fixup · ${kb(String(u.goInto))} files · ${kb(String(c.checkoutCommit))} checkout · ${kb(String(c.copyCommitAttributeToClipboard ?? u.copyToClipboard))} copy · ${kb(String(c.viewBisectOptions))} bisect · ${kb(String(c.viewResetOptions))} reset · ${kb(String(c.revertCommit))}/${kb(String(c.createTag ?? c.tagCommit))} revert/tag`;
    }
