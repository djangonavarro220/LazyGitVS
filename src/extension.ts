...
class LazyGitVSController {
  ...
  attach(panel: ViewPanel, view: vscode.WebviewView) {
    ...
    view.webview.onDidReceiveMessage(async rawMessage => {
      try {
        ...
        if (type === 'checkoutCommitFile') await this.checkoutCurrentCommitFile(); if (type === 'discardCommitFile' && panel === 'commits') await this.discardCurrentCommitFile();
  ...
  private help(panel: Panel): string {
    ...
    if (panel === 'commits') {
      ...
      if (this.commitFilesFor) return common + `${kb(String(cf.checkoutCommitFile))} checkout · ${kb(String(u.remove))} discard from commit · ${kb(String(u.goInto))} open/toggle`;
      return common + `${kb(String(u.remove))} drop · ${kb(String(c.squashDown))} squash · ${kb(String(c.markCommitAsFixup))} fixup · ${kb(String(u.goInto))} files · ${kb(String(c.checkoutCommit))} checkout · ${kb(String(c.copyCommitAttributeToClipboard ?? u.copyToClipboard))} copy · ${kb(String(c.viewBisectOptions))} bisect · ${kb(String(c.viewResetOptions))} reset · ${kb(String(c.revertCommit))}/${kb(String(c.createTag ?? c.tagCommit))} revert/tag`;
    }
