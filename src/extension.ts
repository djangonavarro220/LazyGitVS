...
import { commitRewordMenuItem } from './commitReword';
...
class LazyGitVSController {
  ...
  private commitRewordItem(key: string): GitMenuItem {
    return commitRewordMenuItem({
      key,
      repoPath: workspaceRoot(),
      visibleHashes: this.filteredCommits().map(commit => commit.hash),
      selectedIndex: this.commitSelected,
      range: this.commitRange,
      isLocalCommits: !this.commitListForBranch,
      prompt: async (title, initialSummary) => vscode.window.showInputBox({ title, value: initialSummary, validateInput: value => value.trim() && !/[\r\n]/.test(value) ? undefined : 'Commit summary required.' }),
      onStatus: status => { this.statusLine = status; this.renderAll(); }, onMessage: message => { vscode.window.showWarningMessage(message); },
    });
  }
