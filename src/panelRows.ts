import { type Branch, type ChangedFile, type Commit } from './gitService';
import { type FileTreeRow } from './panels';

export function activeDescendantId(active: boolean, index: number, length: number): string {
  return active && index >= 0 && index < length ? `lgvs-row-${index}` : '';
}
...
export function row(sel: boolean, klass: string, status: string, main: string, meta = '', index?: number): string {
  const data = typeof index === 'number' ? ` id="lgvs-row-${index}" data-index="${index}"` : '';
