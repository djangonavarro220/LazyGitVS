...
assert(extension.includes("if (panel === 'commits') {" ) && extension.includes('this.commitFilesController.active'), 'Commits panel must distinguish commit list vs the controller-owned commit-files subview');
...
assert(extension.includes('const previewToken = this.commitFilesController.beginPreview()') && extension.includes('this.commitFilesController.previewCurrent(previewToken') && extension.includes('owner.repoPath'), 'Moving through files inside a commit must use a captured controller preview token and owner repository, not a stale global cwd');
assert(controllerSource.includes('rows(options = this.treeOptions())'), 'Commit-file drilldown must project its tree through the authoritative controller instead of rendering a flat raw list');
assert(extension.includes("const commitFileRows = this.commitFilesController.rows(this.lazygitGui);"), 'Commits subview should render controller-owned commit files through the same tree/row renderer as 2 Files');
assert(extension.includes('treeFileRow(active && i===this.commitFilesController.selected'), 'Commit-file rows should reuse the same treeFileRow colors/status badges as 2 Files');
...
assert(!extension.includes("showText(`LazyGitVS ${this.commitFilesController.commit.hash}:${f.path}`"), 'Commit-file navigation must not regress to readonly text patch buffers');
...
assert(extension.includes("if (this.readOnlyHunkMode && this.commitFilesController.commit) { this.activePanel = 'commits';"), 'Esc from read-only commit-file HUNK mode must return to the commit-files subview, not jump to Files');
...
assert(extension.includes("if (viewPanel === 'commits' && this.commitFilesController.active) { await this.resetCommitFilesState(false); this.renderAll(); await this.openCurrent('commits', true).catch(() => undefined); return; }"), 'Esc/Back from commit files must invalidate and reconcile the owner before restoring selected commit preview');
