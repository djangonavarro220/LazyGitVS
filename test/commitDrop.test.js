...
(async () => {
  ...
  await test('source routing uses configured universal.remove only for top-level Commits and documents bounded Drop parity', () => {
    ...
    assert(extension.includes('if ((cherryPickBufferAction || dropAction || squashAction) && this.commitFilesFor) return;'), 'Drop must not execute from the commit-files subview');
