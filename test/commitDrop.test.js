...
(async () => {
  ...
  await test('source routing uses configured universal.remove only for top-level Commits and documents bounded Drop parity', () => {
    ...
    assert(extension.includes('if ((cherryPickBufferAction || dropAction) && this.commitFilesFor) return;'), 'Drop must not execute from the commit-files subview');
    ...
    assert(model.includes("cp.execFile('git'"), 'Drop must invoke Git through execFile argv, never a shell command');
    assert(model.includes('GIT_SEQUENCE_EDITOR') && model.includes('LGVS_DROP_HASHES'), 'the private sequence editor must receive selected hashes through environment data');
    assert(model.includes("GIT_EDITOR: 'true'") && model.includes("LC_ALL: 'C'"), 'the rebase must remain non-interactive and locale-stable outside the sequence editor');
    assert(model.includes('0o700') && model.includes("'git-rebase-todo'"), 'the sequence editor must be private and edit only the generated todo');
    assert(model.includes("'--interactive', '--autostash', '--keep-empty', '--no-autosquash', '--rebase-merges'"), 'Drop must use the bounded explicit interactive rebase argv');
