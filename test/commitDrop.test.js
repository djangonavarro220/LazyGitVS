...
(async () => {
  ...
  await test('source routing uses configured universal.remove only for top-level Commits and documents bounded Drop parity', () => {
    ...
    assert(extension.includes("panel==='commits'&&!${this.commitFilesController.commit ? 'true' : 'false'}&&hit(e,u.remove"), 'only the top-level Commits webview route may promote universal.remove into commitAction');
    assert(extension.includes('if ((cherryPickBufferAction || dropAction || squashAction || fixupAction || autosquashAction) && this.commitFilesController.commit) return;'), 'Drop must not execute from the commit-files subview alongside autosquash');
