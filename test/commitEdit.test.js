...
(async () => {
  await test('configured universal e is isolated to top-level Local Commits and documents bounded Edit parity', () => {
    ...
    assert(extension.includes("panel==='commits'&&!${this.commitFilesController.commit ? 'true' : 'false'}&&hit(e,u.remove") && extension.includes('c.amendToCommit,u.edit,c.createFixupCommit'), 'only the top-level Commits webview route may promote configured e into commitAction');
    assert(extension.includes('editAction') && extension.includes('this.commitFilesController.commit) return;'), 'Commit-files must never execute the top-level Edit action');
