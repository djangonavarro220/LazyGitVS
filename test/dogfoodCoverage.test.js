...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("function nativeModalAction(action)") && dogfood.includes("action === 'confirm' ? [352, 64] : [117, 64]") && dogfood.includes("['mousemove', String(point[0]), String(point[1])]") && dogfood.includes("['click', '1']"), 'native modal confirmation must physically move to and click the captured Abort/Cancel buttons as separate XTEST operations');
