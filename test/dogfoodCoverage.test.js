...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes('branchEnterPageText') && dogfood.includes('helpReturnPageText'), 'broad dogfood must evaluate panel ownership against full page text rather than a preview-inflated 3000-character sample');
  assert(dogfood.includes("'selected branch commit list'") && dogfood.includes("'other-repo current status selection'"), 'broad dogfood must wait for physical branch and repository transitions instead of sampling them after a fixed delay');
  const branchesEnterStart = dogfood.indexOf('await selectMasterBranchFixture();');
  const branchesEnterPath = dogfood.slice(branchesEnterStart, dogfood.indexOf("await chord(Input, 'ctrl+alt+4')", branchesEnterStart));
  assert(branchesEnterStart >= 0 && dogfood.includes("async function selectMasterBranchFixture() {\n      await chord(Input, 'ctrl+alt+3');\n      await key(Input, 'Home');") && branchesEnterPath.includes("await chord(Input, 'ctrl+alt+enter');"), 'Branches Enter dogfood must select the deterministic master fixture through the keyboard-only focus/Home path before dispatching Enter');
  assert(!branchesEnterPath.includes('clickWorkbenchPaneRow') && !/Input\.dispatchMouseEvent/.test(branchesEnterPath), 'Branches Enter dogfood must not use a DOM coordinate selector or mouse-event fallback');
  assert(dogfood.includes('const waitForBranchEnter = () => waitFor(') && dogfood.includes('const branchEnterPageText = await waitForBranchEnter();') && branchesEnterPath.includes("'selected branch commit list'"), 'Branches Enter dogfood must verify the keyboard-selected branch through its product-state transition without an interaction-style retry');
  assert(dogfood.includes('await selectOtherRepoStatusFixture();') && dogfood.includes("'other-repo current status selection'") && dogfood.includes("'Status Enter switches from the current repository row to other-repo'"), 'secondary-repository dogfood must use the deterministic keyboard fixture path and verify the selected repository through product state');
  assert(dogfood.includes('await selectFirstCommitFixture();') && dogfood.includes('await selectSecondCommitFixture();') && dogfood.includes('await selectFirstStashFixture();'), 'targeted preview dogfood must select the two commits and stash through deterministic keyboard fixture helpers');
  assert((dogfood.match(/runCommandPalette\(Input, 'LazyGitVS: Focus SCM Sidebar'\)/g) || []).length >= 2 && dogfood.includes('await selectFirstCommitFixture();'), 'targeted preview dogfood must focus the sidebar and deterministically select the primary-repository commit before requiring preview evidence');
  assert(dogfood.includes("await typeText(Input, `>${commandText}`);") && dogfood.includes("await waitForText(Runtime, new RegExp(commandText.replace") && dogfood.includes("await key(Input, 'Enter');") && dogfood.includes("await waitFor(async () => !(await quickInputState(Runtime)).visible, 5000, 100, 'QuickPick selection accepted');"), 'keyboard-only exact-command QuickPick selection must type the unique command, wait until it is visible, press Enter, and verify the QuickPick closes');
  assert(dogfood.includes('await selectSettingsJsonFixture();') && dogfood.includes('settings HUNK mode after primary-repo file click'), 'broad dogfood must restore the primary repo and keyboard-select settings.json before HUNK entry');
  assert(dogfood.includes('richPreviewSnapshots.length === 3') && dogfood.includes('one rich-preview tab after the stash'), 'targeted preview dogfood must keep one rich tab across two keyboard-selected commits and one stash');
  ...
  assert(!dogfood.includes('Input.dispatchMouseEvent') && !dogfood.includes('XTestFakeMotionEvent') && !dogfood.includes('XTestFakeButtonEvent'), 'keyboard-only dogfood must not use CDP or XTEST coordinate mouse input');
  assert(dogfood.includes("await key(Input, 'Escape');") && dogfood.includes("await key(Input, 'Enter');"), 'operation confirmation handling must retain physical keyboard cancel and confirm paths');
