...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  const branchesEnterStart = dogfood.indexOf("await chord(Input, 'ctrl+alt+3')");
  const branchesEnterPath = dogfood.slice(branchesEnterStart, dogfood.indexOf("await chord(Input, 'ctrl+alt+4')", branchesEnterStart));
  assert(branchesEnterPath.includes("clickWorkbenchPaneRow(Runtime, Input, '3 BRANCHES', 'master')") && branchesEnterPath.includes("'physical master branch row'"), 'Branches Enter dogfood must DOM-select the stable master branch row before dispatching Enter');
  assert(!/clickWorkbenchPaneRow\(Runtime, Input, '3 BRANCHES', \d+\)/.test(branchesEnterPath), 'Branches Enter dogfood must not use geometric branch-row indices');
  assert(dogfood.includes('const waitForBranchEnter = () => waitFor(') && dogfood.includes('branchEnterPageText = await waitForBranchEnter().catch(async () => {') && branchesEnterPath.includes("clickWorkbenchPaneRow(Runtime, Input, '3 BRANCHES', 'master')") && branchesEnterPath.includes("'physical master branch row retry'"), 'Branches Enter dogfood must retry and verify the DOM-selected master row before failing');
  assert(dogfood.includes("const secondaryRepoBasename = path.basename(secondaryRepo);") && dogfood.includes("clickWorkbenchPaneRow(Runtime, Input, '1 STATUS', secondaryRepoBasename)") && dogfood.includes("'physical other-repo Status row'") && dogfood.includes("Status selection did not focus/select the exact other-repo basename row") && dogfood.includes("await chord(Input, 'ctrl+alt+enter');"), 'secondary-repository dogfood must DOM-select and verify the visible Status row by exact repository basename before entering it');
  ...
  assert((dogfood.match(/await key\(Input, '0', \{ ctrl: true, alt: true \}\);\n\s+await sleep\(500\);\n\s+await key\(Input, 'Escape'\);/g) || []).length >= 3, 'repo-selection shortcuts must wait for and close the asynchronous Recent repositories quick-pick before continuing');
