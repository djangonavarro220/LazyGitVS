...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("clickWorkbenchPaneRow(Runtime, Input, '1 STATUS', secondaryOperationLabel)") && dogfood.includes("'physical other-repo Status row'") && dogfood.includes("Status selection did not focus/select the exact other-repo operation row") && dogfood.includes("await chord(Input, 'ctrl+alt+enter');"), 'secondary-repository dogfood must DOM-select and verify the exact visible Status row before entering it');
