...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("function nativeModalAction(action)") && dogfood.includes("action === 'confirm' ? [352, 64] : [117, 64]") && dogfood.includes("['mousemove', String(point[0])") && !dogfood.includes("['mousemove', '--sync'"), 'native modal confirmation must physically click the captured Abort/Cancel buttons without waiting for unavailable Xvfb movement synchronization');
