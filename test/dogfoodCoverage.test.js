...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("clickWorkbenchPaneRow(Runtime, Input, '1 STATUS', 1)") && dogfood.includes("'physical other-repo Status row'") && dogfood.includes("await chord(Input, 'ctrl+alt+enter');"), 'secondary-repository dogfood must physically click the visible Status row before entering it');
