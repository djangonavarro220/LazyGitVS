...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("clickWorkbenchPaneRow(Runtime, Input, '3 BRANCHES', 0)") && dogfood.includes("'physical master branch row'"), 'Branches Enter dogfood must focus a visible branch row before dispatching Enter');
  assert(dogfood.includes('const waitForBranchEnter = () => waitFor(') && dogfood.includes('branchEnterPageText = await waitForBranchEnter().catch(async () => {') && dogfood.includes("'physical master branch row retry'"), 'Branches Enter dogfood must retry the physical branch-row click before failing');
