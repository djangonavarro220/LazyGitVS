...
test('dogfood asserts the documented visible UI smoke path', () => {
  ...
  assert(dogfood.includes("function nativeModalAction(action)") && dogfood.includes("action === 'confirm' ? [352, 64] : [117, 64]") && dogfood.includes("['getmouselocation', '--shell']") && dogfood.includes("['mousedown', '--window', targetWindow, '1']") && dogfood.includes("['mouseup', '--window', targetWindow, '1']"), 'native modal confirmation must physically press the captured Abort/Cancel button window under the moved pointer');
