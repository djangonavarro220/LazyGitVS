...
let projectCalls = 0;
const countedBase = { ...base, project: rows => { projectCalls++; return [...rows]; } };
const first = model.read(countedBase);
const firstRowIdentities = [...first];
const moved = model.read({ ...countedBase, selection: 1 });
assert.strictEqual(moved, first, 'production selection reads reuse the published rows');
assert.deepStrictEqual(moved.map((row, index) => row === firstRowIdentities[index]), moved.map(() => true), 'production selection reads allocate zero new rows');
assert.strictEqual(projectCalls, 1, 'production selection reads keep one published snapshot without rebuilding');
