...
assert(extension.includes('.status-pair{display:grid;grid-template-columns:9px 9px;column-gap:0;align-items:center;font-family:var(--vscode-editor-font-family);font-size:11px}'), 'Files short-status letters should render as one compact two-character block, not spaced-out floating badges');
assert(extension.includes('.slot{display:inline-grid;place-items:center;min-width:9px;height:14px;border-radius:0;font-size:11px;font-weight:600;line-height:1;border:0;background:transparent;box-sizing:border-box}'), 'Files short-status letters should look like compact VS Code SCM text, not pill/circular badges');
assert(extension.includes('.row.sel .slot{color:inherit}'), 'Selected Files rows should keep the status letters legible on the blue selection background');
...
assert(extension.includes('.row.file{grid-template-columns:7px 26px minmax(0,1fr);}'), 'Files rows should use a compact short-status column and give the rest to the path');
