...
const focusMethod = extension.slice(extension.indexOf('  async focus() {'), extension.indexOf('  async focusNumberPanel'));
assert(!extension.includes('defaultPanelsRevealed'), 'manifest visibility, not runtime focus-all state, controls the default-open contributed views');
assert(!extension.includes('revealDefaultOpenPanels'), 'openDashboard must not serially reveal or focus panels 2-8 before Files');
assert(focusMethod.includes('return this.focusPanel(this.activeViewPanel(), bootstrap);'), 'openDashboard must directly focus the active Files panel after loading configuration');
...
assert(focusRequest.includes('class FocusRequestStateMachine'), 'focus readiness must be governed by a deterministic state machine');
assert(focusRequest.includes('renderedGenerations'), 'state machine must track each rendered webview generation');
assert(focusRequest.includes('renderedGenerations.get(panel) !== generation'), 'stale ready/render generations must be rejected');
assert(focusRequest.includes('pending.request !== request'), 'an old physical focus acknowledgement must not complete a newer request');
assert(extension.includes('this.pendingPanelFocusTransition.begin(panel, transition?.from ?? previousViewPanel);'), 'a real panel transition must become pending before asynchronous refresh, preview, or reveal work');
assert(extension.includes("(rawMessage as { type?: unknown }).type === 'ready'"), 'the Extension Host must receive a distinct ready protocol message before normal focus-area handling');
assert(extension.includes('this.pendingPanelFocusTransition.ready(panel, generation)'), 'only matching rendered readiness may dispatch focusBody');
assert(extension.indexOf("window.addEventListener('message'") < extension.indexOf("vscode.postMessage({type:'ready',generation})"), 'ready must be posted only after the webview message listener is installed');
assert(extension.indexOf('function markPanelFocus(request)') < extension.indexOf("vscode.postMessage({type:'ready',generation})"), 'ready must be posted only after markPanelFocus is installed');
assert(extension.includes("void view.webview.postMessage({ type: 'focusBody', request: request.request, generation: request.generation })"), 'ready may dispatch focusBody without treating postMessage settlement as acknowledgement');
assert(extension.includes('this.pendingPanelFocusTransition.acknowledge(panel, Number(message.generation), Number(message.request))'), 'only the correlated physical focusArea message may complete the request');
assert(extension.includes("recordDogfoodBoundary('panelFocus', { from: transition.from, to: transition.panel"), 'panelFocus remains the sole authoritative physical acknowledgement');
assert(!extension.includes('focusWorkflowAfterFinalReveal'), 'final reveal is not an acknowledgement');
assert(!extension.includes('focusBodyPostMessageSettled'), 'postMessage settlement is not an acknowledgement');
assert(!extension.includes('await settleFocusRequest(focusBody)'), 'focusBody must never wait for a synthetic acknowledgement');
assert(extension.includes("vscode.postMessage({type:'ready',generation})"), 'webview posts ready only after installing the message listener and markPanelFocus');
assert(extension.includes("event.data.type==='focusBody'&&event.data.generation===generation"), 'webview only applies focusBody to its current rendered generation');
assert(extension.includes('document.body.focus();markPanelFocus(event.data.request);'), 'focusBody retains physical body focus before correlated panelFocus acknowledgement');
