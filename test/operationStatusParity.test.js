...
assert(dogfood.includes('const secondaryRepoBasename = path.basename(secondaryRepo);') && dogfood.includes("clickWorkbenchPaneRow(Runtime, Input, '1 STATUS', secondaryRepoBasename)"), 'operation Status selection must target the secondary-repository row by basename DOM text/title, not a dynamic operation label or geometric row index');
const operationSelection = dogfood.indexOf('const selectedSecondaryStatusRow =');
assert(dogfood.includes('Status selection did not focus/select the exact other-repo basename row') && operationSelection >= 0 && dogfood.indexOf("await chord(Input, 'ctrl+alt+enter');", operationSelection) > operationSelection, 'operation flow must verify the basename-matched Status row is selected immediately before Enter');
